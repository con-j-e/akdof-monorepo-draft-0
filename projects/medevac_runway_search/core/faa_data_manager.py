...
class FaaDataManager():
    """
    Manages data retrieval, new data identification, and data caching. Logic is absolutely dependent on `FaaDataSheetsConfig`.
    As implemented by this project, `FaaDataManager()` triggers entire target feature layer overwrites.
    Given how rarely updates occur this simplistic approach makes sense.
    In another context, the `fresh_data` and `deleted_data` attributes could be used in an approach that updates only changed records.
    """

    def __init__(self, sheets_config: dict, proj_dir: Path):
    ...
    def refresh_data(self):
        """Loads all current FAA data specified by `self.sheets_config`. Saves fresh data, complete data, and deleted data to instance attributes."""

        url = self.sheets_config["url"]
        state_of_interest = self.sheets_config["state_of_interest"]
        sheets = self.sheets_config["sheets"]
        for sheet_name, config in sheets.items():

            # load current data and filter/format to create complete dataframe specified by `sheets_config`
            complete_df = pd.read_excel(io=url, sheet_name=sheet_name)
            complete_df = complete_df[complete_df[config["state_column"]] == state_of_interest]
            complete_df = complete_df[config["columns_to_keep"]]
            complete_df = complete_df.rename(columns={c: c.replace(" ", "_").lower() for c in complete_df.columns})
            complete_df = complete_df.set_index(config["index"], drop=False, verify_integrity=True)
            self.complete_data[sheet_name] = complete_df

            self._assert_interchangeable_loc_id_site_id()

            # load cached data
            with open(self.proj_dir / config["cached_csv"], "r") as file:
                cache_df = pd.read_csv(file)
            cache_df = cache_df.set_index(config["index"], drop=False, verify_integrity=True)
    ...
    def update_cache(self):
        ...
        for sheet_name, config in sheets.items():
            ...
            with open(self.proj_dir / config["cached_csv"], "w", newline="") as file:
    ...
    def _get_related_records(self):
        ...
        fresh_site_ids = set()
        for _, fresh_df in self.fresh_data.items():
